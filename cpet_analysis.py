...
from utils import DATA_TYPE, META_TYPE, RESULTS_TYPE, RESULTS_INFO_TYPE
from utils.data_reader import (
    find_valid_name_wrapper, meta_data_iterator, read_study_data, read_without_meta, fix_time
)
from utils.calculations import run_analysis
from utils.output import save_spreadsheet, plot_data_time, plot_data_nondimensional, update_spreadsheet
...
def _update_plot_settings(**updates: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    for kw in updates:
        if kw not in plotting_params:
            raise NotImplementedError(f"Unable to process '{kw}': '{plotting_params[kw]}'")
...
def use_supplied_meta(
    ...
    data format.
    """
    find_valid_name = find_valid_name_wrapper(
        input_path=input_path, meta_path=meta_path
    )
    ...
    for meta in tqdm(iterator, desc="Analyzing", total=n):
        ...
        data, units, _ = read_study_data(
            file, layout=layout, sheet_name=sheet_name, 
        ...
        if plot:
            plot_study(data=data, study=study, extra_info=extra_info, output_dir=output_dir, **plot_settings)
...
def use_inferred_meta(
    ...
    if input_path.is_file() and input_path.suffix in [".xlsx", ".xls"]:
        ...
        raise FileNotFoundError(f"Could not read spreadsheet with extension '{input_path.suffix}'")
    ...
    for name in tqdm(sheet_names, desc="Analyzing", total=len(sheet_names)):
        ...
        meta, data, units = read_without_meta(path, sheet_name=sheet_name, layout=layout, recalculate_o2_pulse=recalculate_o2_pulse, plot=plot)
        #FIXME: verify units are correct
